...
export function createPrismaticBurst(container, options = {}) {
  const {
    intensity = 2,
    speed = 0.5,
  ...
  if (Array.isArray(colors) && colors.length > 0) {
    const count = Math.min(colors.length, 64);
  ...
  return {
    destroy() {
      ...
      try { triangle.remove(); } catch (e) { /* ignore */ }
      try { program.remove(); } catch (e) { /* ignore */ }
      try {
        const glCtx = renderer.gl;
        if (glCtx && gradientTex.texture) glCtx.deleteTexture(gradientTex.texture);
      } catch (e) { /* ignore */ }
      renderer.destroy();
    },
    setPaused(p) { paused = p; },
    setIntensity(v) { program.uniforms.uIntensity.value = v; },
    setSpeed(v) { program.uniforms.uSpeed.value = v; }
  };
}
